/*
*title:token handler
*Description:Handler to handle token reated routes
*Author:
*Date:
*/
...
handler._token.get = (requestProperties, callback) => {
    ...
    requestProperties.queryStringObject.id.trim().length === 20 ? requestProperties.queryStringObject.id : false;
    if(id){
    //lookup the token
        data.read('tokens', id, (err, tokenData) => {
        const token = { ...parseJSON(tokenData)};
        /*
        { name: 'jasshi', age: 23, gender:'male'}
        */
           if(!err && token){
            callback(200, token);
            }else{
          ...
          if(!err1 && tokenData){
              data.delete('tokens', id, (err2) => {
                if(!err2){
                    ...
                    callback(500, {
                        error: 'There was a srver side error!',
                    }); 
                }
              });
